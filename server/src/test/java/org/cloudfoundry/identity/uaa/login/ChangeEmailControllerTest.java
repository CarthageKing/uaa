...
public class ChangeEmailControllerTest extends TestClassNullifier {
    ...
    public void testVerifyEmailWhenAutheticatedAsOtherUser() throws Exception {
        UaaUser user = new UaaUser("user-id-002", "new2@example.com", "password", "new2@example.com", Collections.<GrantedAuthority>emptyList(), "name", "name", null, null, OriginKeys.UAA, null, true, IdentityZoneHolder.get().getId(),"user-id-002", null, null);
        when(uaaUserDatabase.retrieveUserById(anyString())).thenReturn(user);
